from model.table import TableAbstract


class CommentsTable(TableAbstract.TableAbstract):
    """
        CommentsTable is a class for querying Comments Table
    """

    # def __init__(self):
    #     TableAbstract.TableAbstract.__init__(self)
    ...
    def selectCommentByLimitOrderBy(self, limit, order):
        sql = '''
			SELECT *
			  FROM comment
			 ORDER BY {0}
        ...
        sql = sql.format(orderStr, str(limit))
...
# print(newCommentsTable.selectCommentByLimitOrderBy(5, [('id', 'DESC')]))
